  ...
  uniqueAthletesList.forEach(
    function(athlete) {
      ...
      if (!allSheets.includes(athlete)) {
        ...
        newAthleteSheet.getRange("H2:H10000").setNumberFormat(`h":"mm" "am/pm`); // format for time
        ...
        if (athleteActivities.length > 0) {
          ...
          if (athleteSheetWeeksWorkouts.length > 0) {
            ...
            newAthleteSheet.getRange(1 + athleteActivities.length + ROWS_DOWN_FROM_LAST_RUN_FOR_WORKOUT_STATS, 7, athleteSheetWeeksWorkouts.length, 1).setNumberFormat("hh:mm AM/PM");
...
function getAthleteSheets() {
  var nonAthleteSheets = ["ME", "TO DO / THOUGHTS", "GOONS", "GOONS RECAP"];
