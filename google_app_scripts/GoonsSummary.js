...
function generateGoonsRecapSheet() {
  ...
  function populateGoonsRecapSheet(summariesList, workoutSummariesList) {
    ...
    /* BASIC RECAP STATS */ 
    // If we have contents in our summariesList, handle this section
    ...
    } else Logger.log(`There are no entries in the summariesList returned from getWeekSummaries().`);
  
    /* WORKOUT RECAP STATS */ 
    // If have workout(s) we will handle that section
    if (workoutSummariesList.length > 0) {
      // Headers and borders 
      ...
      goonsRecapSheet.getRange(ROW_OF_WEEKLY_RECAP_HEADER_RECAP_SHEET + summariesList.length + ROWS_DOWN_FROM_LAST_WEEKLY_RECAP_ROW_STATS, 1, workoutSummariesList.length, workoutSummariesList[0].length).setValues(workoutSummariesList).sort([{column: 1, ascending: true}, {column: 2, ascending: true}]);
    } else Logger.log(`There were no workouts returned from getWorkoutSummaries().`);
  ...
  function getWorkoutSummaries() {
    ...
    getAllSheets().filter((sheet) => getListOfUniqueAthletes(SpreadsheetApp.getActiveSpreadsheet().getSheetByName("GOONS")).includes(sheet) && SpreadsheetApp.getActiveSpreadsheet().getSheetByName(sheet).getRange(2,1).getValue()).forEach((athleteSheetName) => {
      ...
      if (numWorkouts > 0)  {
        ...
        athleteSheet.getRange(1 + getActivitiesFromAthleteSheet(athleteSheet).length + ROWS_DOWN_FROM_LAST_RUN_FOR_WORKOUT_STATS, 1, numWorkouts, NUM_OF_WORKOUT_RECAP_STATS).getValues().forEach((wktRecapStat) => {
          wktRecapStat.unshift(athleteSheetName);
          wktRecapStats.push(wktRecapStat);
        });
