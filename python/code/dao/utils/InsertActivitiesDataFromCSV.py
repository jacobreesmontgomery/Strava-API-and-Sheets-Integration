...
import StravaActivitiesDao
import StravaAthleteDao
    ...
    with open(csv_file_path, mode='r') as file:
        ...
        for row in reader:
            ...
            activity_data = (
                ...
                int(row[14]) if row[14] else "N/A",                # wkt_type
                row[15],          # description
                ...
                int(row[25]) if row[25] != "N/A" else row[25],     # rpe (int or "N/A")
                int(row[26]) if row[26] != "N/A" else row[26],     # rating (int or "N/A")
                int(row[27]) if row[27] != "N/A" else row[27],     # avg_power (int or "N/A")
                int(row[28]) if row[28] != "N/A" else row[28]      # sleep_rating (int or "N/A")
            )
