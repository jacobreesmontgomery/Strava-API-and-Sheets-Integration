...
class StravaAPI:
    ...
    def fetch_activities(self, start_date: str = None, end_date: str = None) -> list[Activity] | None:
        """
        ...
        try:
            ...
            self.logger.error(f"Failed to retrieve activities on final retry attempt [{e.last_attempt.attempt_number}]: {e}")
            return None
    ...
    def fetch_detailed_activity(self, activity_id: int) -> Activity | None:
        ...
        try:
            ...
            self.logger.error(f"Strava API rate limit exceeded for activity [{activity_id}]: {e}")
            return None
        except RetryError as e:
            self.logger.error(f"Failed to retrieve detailed activity [{activity_id}] on final retry attempt [{e.last_attempt.attempt_number}]: {e}")
            return None
        except Exception as e:
            self.logger.error(f"Failed to retrieve detailed activity with ID [{activity_id}]: {e}")
            raise ActivityRetrievalException(f"Failed to retrieve detailed activity with ID [{activity_id}]: {e}")
    ...
    def get_activities_this_week(self) -> list[Activity]:
        ...
        activities = self.fetch_activities(start_date=start_of_week, end_date=end_of_week)
        if not activities:
    ...
    def get_activities(self, athlete_id: int, start_date: str = None, end_date: str = None) -> list[Activity]:
        ...
        try:
            ...
            self.logger.error(f"Failed to retrieve activities for athlete ID {athlete_id}: {e}")
            return None
